...
class RecordEpisode(gym.Wrapper):
    ...
    def flush_trajectory(self, verbose=False, ignore_empty_transition=False):
        ...
        if isinstance(obs[0], dict):
            ...
            # NOTE: If each obs is empty, then nothing will be stored.
            ...
            for k, v in obs.items():
                ...
                if "rgb" in k and v.ndim == 4:
                    # NOTE: It is more efficient to use gzip than png for a sequence of images.
                    ...
                    # NOTE: uint16 is more efficient to store at cost of precision
        ...
        if len(self._episode_data) == 1:
            ...
            # NOTE: The format is designed to be compatible with ManiSkill-Learn (pyrl).
            # Record transitions (ignore the first padded values during reset)
            actions = np.stack([x["a"] for x in self._episode_data[1:]])
            # NOTE: "dones" need to stand for task success excluding time limit.
