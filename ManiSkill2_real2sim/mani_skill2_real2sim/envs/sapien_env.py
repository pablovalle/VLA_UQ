...
class BaseEnv(gym.Env):
    ...
    def __init__(
        ...
        # TODO: Change to `warning` after lighting in VecEnv is fixed.
        ...
        if self._renderer_type == "sapien":
            ...
            # TODO: add `set_log_level` for RenderClient?
        else:
        ...
        # TODO: Support dict action space
        if control_mode == "*":
    ...
    def _configure_agent(self):
        # TODO: Support a dummy agent for simulation only
    ...
    def reconfigure(self, options):
        ...
        camera_cfgs = options.get("camera_cfgs", None)
        render_camera_cfgs = options.get("render_camera_cfgs", None)
        if camera_cfgs:
            update_camera_cfgs_from_dict(self._camera_cfgs, camera_cfgs)
        if render_camera_cfgs:
            update_camera_cfgs_from_dict(self._render_camera_cfgs, render_camera_cfgs)
        self._setup_cameras()
    ...
    def get_articulations(self):
        ...
        # NOTE: There might be dummy articulations used by controllers.
        # TODO: Remove dummy articulations if exist.
