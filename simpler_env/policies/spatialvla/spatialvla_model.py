...
class SpatialVLAInference:
    def __init__(
        self,
        saved_model_path: str = "IPEC-COMMUNITY/spatialvla-4b-mix-224-pt",
    ...
    def step(
        ...
        with torch.no_grad():
            if hasattr(self.processor, "action_tokenizer"):
                # print(type(self.vla))
                generation_outputs, scores = self.vla.predict_action(inputs)
                
                import inspect
                function_path = inspect.getfile(self.vla.predict_action)
                # print(f"Path of 'my_function': {function_path}")
                raw_actions = self.processor.decode_actions(
                    generation_outputs=generation_outputs,
                    unnorm_key=self.unnorm_key,
                )["actions"]
            else:
                raw_actions, scores = self.vla.predict_action(**inputs)["actions"]
        ...
        raw_action = {
            ...
            "scores": scores,
            #"scores": scores  # range [0, 1]; 1 = open; 0 = close
        }
