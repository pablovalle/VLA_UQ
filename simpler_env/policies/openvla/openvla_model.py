...
class OpenVLAInference:
    ...
    def _resize_image(self, image: np.ndarray) -> np.ndarray:
        ...
        # Resize with LANCZOS (equivalent to lanczos3 in TF)
        image = image.resize((self.image_size, self.image_size), resample=Image.LANCZOS)
    ...
    def step(self, image: np.ndarray, task_description: Optional[str] = None, *args, **kwargs) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray]]:
        ...
        raw_actions, scores = self.model.predict_action(**inputs, unnorm_key=self.dataset_id, do_sample=False)
        if self.action_ensemble:
